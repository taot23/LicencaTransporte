...
app.patch('/api/admin/licenses/:id/status', requireOperational, upload.single('licenseFile'), async (req, res) => {
  ...
  app.get("/api/gamification/profile", requireAuth, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Erro ao carregar perfil de gamificação" });
    }
  });

  // Mark achievements as viewed
  app.post("/api/gamification/achievements/mark-viewed", requireAuth, async (req, res) => {
    try {
      const userId = req.user!.id;
      const { achievementIds } = req.body;
      const { gamificationService } = await import("./gamification-service");
      
      await gamificationService.markAchievementsViewed(userId, achievementIds);
