...
export class DatabaseStorage implements IStorage {
  ...
  async getLicenseRequestById(id: number): Promise<LicenseRequest | undefined> {
    ...
    // Retornar dados exatamente como estão no banco
    return license;
  }
  ...
  async getIssuedLicensesByUserId(userId: number): Promise<LicenseRequest[]> {
    ...
    console.log(
      `[DEBUG] getIssuedLicensesByUserId: Obtidas ${allLicenses.length} licenças não-rascunho`,
    );
    ...
    const issuedLicenses = allLicenses.filter((license) => {
      ...
      if (license.status === "approved") {
        console.log(
          `[DEBUG] Licença #${license.id} incluída - status principal approved`,
        );
        return true;
      }
      ...
      if (license.stateStatuses && Array.isArray(license.stateStatuses)) {
        ...
        if (hasApprovedState) {
          console.log(
            `[DEBUG] Licença #${license.id} incluída - tem pelo menos um estado approved`,
          );
          return true;
        }
      }
      ...
      console.log(
        `[DEBUG] Licença #${license.id} excluída - não atende aos critérios`,
      );
      return false;
    });

    console.log(
      `[DEBUG] getIssuedLicensesByUserId: Retornando ${issuedLicenses.length} licenças emitidas`,
    );
  ...
  async getAllIssuedLicenses(): Promise<LicenseRequest[]> {
    ...
    console.log(
      `[DEBUG] getAllIssuedLicenses: Obtidas ${allLicenses.length} licenças não-rascunho`,
    );
    ...
    console.log(
      `[DEBUG] getAllIssuedLicenses: Retornando ${issuedLicenses.length} licenças emitidas`,
    );
    return issuedLicenses;
  }
  ...
  async getDashboardStats(userId: number): Promise<DashboardStats> {
    console.log(
      `[DASHBOARD STATS] Obtendo estatísticas para usuário ${userId}`,
    );
    ...
    console.log(
      `[DASHBOARD STATS] Usuário ${user.email} (${user.role}) - É admin: ${isAdmin}`,
    );

    if (isAdmin) {
      // Admin vê todas as estatísticas globais do sistema
      console.log(
        `[DASHBOARD STATS] Coletando estatísticas globais para admin`,
      );
      ...
      console.log(
        `[DASHBOARD STATS] Estatísticas globais para admin:`,
        adminStats,
      );
      return adminStats;
    } else {
      ...
      console.log(
        `[DASHBOARD STATS] TRANSPORTADOR - Coletando estatísticas específicas para usuário ${userId}`,
      );
      ...
      console.log(
        `[DASHBOARD STATS] TRANSPORTADOR - Total de licenças encontradas: ${userLicenses.length}`,
      );
      ...
      console.log(
        `[DASHBOARD STATS] TRANSPORTADOR - Licenças emitidas: ${issuedLicenses.length}, pendentes: ${pendingLicenses.length}`,
      );
      console.log(
        `[DEBUG STATS] IDs das licenças emitidas:`,
      ...
      console.log(
        `[DASHBOARD STATS] TRANSPORTADOR - Veículos registrados: ${registeredVehiclesCount}, ativos: ${activeVehiclesCount}`,
      );
      ...
      const userExpiringLicenses = issuedLicenses.filter((license) => {
        console.log(
          `[DEBUG EXPIRING] Verificando licença ${license.id} - ${license.requestNumber}`,
        );
        ...
        if (license.stateStatuses && license.stateStatuses.length > 0) {
            ...
            if (status.includes(":approved:")) {
              ...
              if (parts.length >= 3) {
                ...
                const isExpiring =
                  validityDate >= today && validityDate <= thirtyDaysFromNow;
                console.log(
                  `[DEBUG EXPIRING] É licença a vencer? ${isExpiring}`,
                );
                return isExpiring;
              }
            }
          ...
          console.log(
            `[DEBUG EXPIRING] Licença ${license.id} tem estado expirando? ${hasExpiringState}`,
          );
        ...
        if (license.validUntil) {
          ...
          console.log(
            `[DEBUG EXPIRING] Fallback validUntil: ${validUntilDate}, é expirando? ${isExpiring}`,
          );
          return isExpiring;
        }
      ...
      console.log(
        `[DASHBOARD STATS] TRANSPORTADOR - Estatísticas FINAIS para usuário ${userId}:`,
        userStats,
      );
      return userStats;
    }
